<?php
    class MinecraftServerStatus {
        ...
        public function getStatus($host = '127.0.0.1', $port = 25565, $version = '1.8.*') {
            ...
            if(preg_match('/1.7|1.8|1.9|1.10|1.11|1.12/',$version)) {
                ...
                if($serverdata['players'] == 0){
                    array_push($playersArray, array('It looks like nobody is online right now!'));
                    ...
                    for($i = 0; $plist = $data->players->sample[$i]->name; $i++){
                        ...
                        if($i == $serverdata['players']-1){
                            array_push($playersArray, $this->add_player($plist, $data->players->sample[$i]->id) );
                            break;
                        }else if($i == 11 && empty($data->players->sample[12]->name)){
                            array_push($playersArray, $this->add_player($plist, $data->players->sample[$i]->id) );
                            array_push($playersArray, $this->add_player("--- and " . ($serverdata['players']-12) . " more ---", 'null'));
                            break;
                    ...
                    array_push($playersArray, array("Unfortunately, this server is hiding their player list."));
                }
        ...
        private function add_player($player, $uuid){
            return array(
                'name' => $player,
                'uuid' => $uuid
            );
